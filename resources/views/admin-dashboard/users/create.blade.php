...
@section('content')
    <div class="p-4 sm:p-6 lg:p-8 max-w-2xl mx-auto">
        ...
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Add User</h1>
                <p class="text-sm text-gray-400 mt-0.5">Create a new user account</p>
            </div>
        </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Email
                            Address</label>
                        <input type="email" name="email" value="{{ old('email') }}" placeholder="e.g. priya@email.com"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select name="role" required
                            class="w-full px-4 py-2.5 rounded-xl text-sm border {{ $errors->has('role') ? 'border-red-300 bg-red-50' : 'border-purple-100 bg-purple-50/30' }} text-gray-600 focus:outline-none focus:border-purple-400 focus:bg-white transition-colors">
                    ...
                    <div class="sm:col-span-2">
                        <label
                            class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-1.5">Phone</label>
...
@push('scripts')
    <script>
        ...
        document.querySelector('[name="name"]').addEventListener('input', function() {
            const initials = this.value.trim().split(' ').filter(Boolean).slice(0, 2).map(w => w[0].toUpperCase())
                .join('');
