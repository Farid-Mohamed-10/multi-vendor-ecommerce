...
<header class="bg-white border-b border-purple-50 sticky z-40">
    <div class="flex items-center px-3 sm:px-6 py-3 gap-3 sm:gap-5">
        ...
        <a href="{{ url('/') }}" class="flex items-center gap-2 flex-shrink-0">
            ...
            <span class="text-xl sm:text-2xl font-extrabold logo-text">ALOSTORA</span>
        </a>
        ...
        <a href="{{ route('front.cart.show') }}"
            class="relative p-2 rounded-xl hover:bg-purple-50 transition-colors flex-shrink-0">
            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z" />
            </svg>
            <span
                class="absolute -top-1 -right-1 w-4 h-4 bg-pink-500 text-white text-xs rounded-full flex items-center justify-center font-bold">{{ count(session('cart', [])) }}</span>
        </a>
            ...
            @if (Auth::check())
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    ...
                    <button @click="open = !open"
                        ...
                        {{-- Avatar Icon --}}
                        <div
                            class="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0 overflow-hidden">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-150"
                        ...
                        <div class="px-4 py-3 bg-gradient-to-r from-indigo-50 to-purple-50 border-b border-gray-100">
                            <div class="flex items-center gap-2.5">
                                <div
                                    class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white text-sm font-bold flex-shrink-0 overflow-hidden">
                                    @if (Auth::user()->profile_photo_path)
                                        <img src="{{ asset('storage/' . Auth::user()->profile_photo_path) }}"
                                            class="w-full h-full object-cover" alt="">
                                    @else
